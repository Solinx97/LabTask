...
public class GetLinksByUserIdTests
{
    [Fact]
    public async Task Handle_Links_ShouldReturnLinksByUserId()
    {
        // Arrange
        ...
        mockRepository.Setup(m => m.GetByUserIdAsync(It.IsAny<Guid>(), It.IsAny<int>(), It.IsAny<int>(), It.IsAny<CancellationToken>())).ReturnsAsync(links);
        mockMapper.Setup(m => m.Map<IEnumerable<LinkDto>>(It.IsAny<IEnumerable<Link>>())).Returns(linksDto);

        var handler = new GetLinksByUserIdHandler(mockRepository.Object, mockMapper.Object);

        // Act
        var result = await handler.Handle(new GetLinksByUserIdQuery(userId, page, pageSize), CancellationToken.None);
        ...
        // Verify
        mockRepository.Verify(m => m.GetByUserIdAsync(It.IsAny<Guid>(), It.IsAny<int>(), It.IsAny<int>(), It.IsAny<CancellationToken>()), Times.Once);
